...
def align_annotators_in_cluster(annotators_in_cluster, alignment, entity_type, annotators_in_document, file, cluster_id):
    ...
    """
        creates cell alignment for excel outputt

        param:
            annotators_in_cluster: list of strings
            alignment: dictionary containing the information of each annotators
...
def align_annotators_in_cluster_classification(annotators_in_cluster, alignment, entity_type, annotators_in_document, file, cluster_id):
  
    """
        creates cell alignment for excel output under the classification menu
...
def align_all_clusters_for_classification(doc_clusters, types, file, all_annotators):
    ...
    for i in range(len(doc_clusters)):
        #for _type in types:
        # filter by type
        current_cluster = [anno for anno in doc_clusters[i]]
        ...
        if current_cluster: # only perform this section if cluster has annotations
            ...
            align_annotators_in_cluster_classification(annos_clus, alignment, 
...
def determine_discrepancies_for_classification(dic):
    ...
    class_discrepancies = []
    class_discrepancy_print = ''

    all_class_discrepancies = []
    all_feature_discrepancies = []
    all_mention_discrepancies = []

    rows = [dic[k] for k in dic if k not in ['file', 'cluster']]
    cluster_rows = [dic[k] for k in dic if k in ['cluster']]
    entity_rows = [dic[k] for k in dic if k in ['entity_type']]
    ...
    for y in range(alignment_length):
        ...
        #return '{}'.format(temp_row)
        #class_discrepancy_print = 'class discrepancy\n'

        if len(list(set([x['mention'] for x in temp_row]))) > 1:
            class_discrepancies.append('class discrepancies')
            class_discrepancy_print += 'class discrepancies\n'
        ...
        all_mention_discrepancies.append(discrepancy_print)
        all_feature_discrepancies.append(feature_discrepancy_print)
        all_class_discrepancies.append(class_discrepancy_print)
    ...
    dic['text_discrepancy'] = all_mention_discrepancies
    dic['feature_discrepancy'] = all_feature_discrepancies
    dic['class_discrepancy'] = all_class_discrepancies


    return {'text_discrepancy': mention_discrepancy, 
            'feature_discrepancy': feature_discrepancies, 
            'class_discrepancy' : class_discrepancies}
...
def format_cells_classification(dict_):
    rows = [dict_[k] for k in dict_ if k not in ['entity_type', 'file', 'cluster', 'text_discrepancy', 'feature_discrepancy', 'class_discrepancy']]
